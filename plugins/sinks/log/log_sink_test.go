...
func TestSimpleWrite(t *testing.T) {
	...
	batch := metrics.DataBatch{
		Timestamp:  now,
		MetricSets: make(map[string]*metrics.MetricSet),
	}
	batch.MetricSets["pod1"] = &metrics.MetricSet{
		...
		LabeledMetrics: []metrics.LabeledMetric{
			{
				Name: "lm",
...
func TestSortedOutput(t *testing.T) {
	...
	metricVal := metrics.MetricValue{
		ValueType:  metrics.ValueInt64,
		MetricType: metrics.MetricGauge,
		IntValue:   31415,
	}
	metricSet := func(pod string) *metrics.MetricSet {
