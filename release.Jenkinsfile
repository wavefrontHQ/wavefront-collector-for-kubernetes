...
//       }
      ...
      stage("Re-tag Harbor Image") {
        ...
        steps {
//           sh 'echo $HARBOR_CREDS_PSW | docker login "projects.registry.vmware.com/tanzu_observability" -u $HARBOR_CREDS_USR --password-stdin'
//           sh 'HARBOR_CREDS_USR=$(echo $HARBOR_CREDS_USR | sed \'s/\\$/\\$\\$/\') docker pull ${PREFIX}/${DOCKER_IMAGE}:${VERSION}-rc-${RC_NUMBER}'
//           sh 'HARBOR_CREDS_USR=$(echo $HARBOR_CREDS_USR | sed \'s/\\$/\\$\\$/\') docker image tag ${PREFIX}/${DOCKER_IMAGE}:${VERSION}-rc-${RC_NUMBER} ${PREFIX}/${DOCKER_IMAGE}:${VERSION}'
