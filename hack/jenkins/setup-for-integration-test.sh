...
if ! [ -x "$(command -v jq)" ]; then
  mkdir -p "$HOME/bin"
  curl -v -H "Authorization: token ${GITHUB_CREDS_PSW}" -L https://github.com/stedolan/jq/releases/download/jq-1.6/jq-linux64 --output "$HOME/bin/jq"
  chmod +x "$HOME/bin/jq"
fi
...
if ! [ -x "$(command -v kustomize)" ]; then
  curl -v -H "Authorization: token ${GITHUB_CREDS_PSW}" -s "https://raw.githubusercontent.com/kubernetes-sigs/kustomize/master/hack/install_kustomize.sh"  | bash
  chmod +x ./kustomize
