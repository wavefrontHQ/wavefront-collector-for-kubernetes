...
func (h *CollectorRunOptions) AddFlags(fs *pflag.FlagSet) {
	...
	fs.DurationVar(&h.FlushInterval, "Push_interval", 60*time.Second, "The interval at which the collector will send metrics to the skin")
	fs.DurationVar(&h.DefaultCollectionInterval, "Default_Collection_Interval", 60*time.Second, "Default Collection Interval for metrics sources")
