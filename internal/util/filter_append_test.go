...
func TestFilterAppend(t *testing.T) {
	t.Run("nil filter acts like append", func(t *testing.T) {
		somePoint := wf.NewPoint("some.metric", 1.0, 2, "pod-123", map[string]string{"foo": "bar"})
		incrementer := fakeCounter(0)

		points := FilterAppend(nil, &incrementer, []*wf.Point{}, somePoint)
	...
	t.Run("filters metrics", func(t *testing.T) {
		...
		points := FilterAppend(filters, &incrementer, []*wf.Point{}, somePoint)
	...
	t.Run("filters tags on appended metrics", func(t *testing.T) {
		somePoint := wf.NewPoint("some.metric", 1.0, 2, "pod-123", map[string]string{"foo": "bar", "bar": "foo"})
		incrementer := fakeCounter(0)
		filters := filter.NewGlobFilter(filter.Config{
