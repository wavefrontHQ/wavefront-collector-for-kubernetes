...
func TestParseMetric(t *testing.T) {
	t.Run("can parse histograms", func(t *testing.T) {
		metric, err := ParseMetric("!M 1493773500 #20 30 #10 5 request.latency source=\"appServer1\" region=\"us-west\"")
		assert.Nil(t, metric)
		assert.NoError(t, err)
	})

	t.Run("can parse metrics", func(t *testing.T) {
		metric, err := ParseMetric("system.cpu.loadavg.1m 0.03 1382754475 source=\"test1.wavefront.com\"")
		assert.NoError(t, err)
